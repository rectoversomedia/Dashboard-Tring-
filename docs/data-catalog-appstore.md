...
- **ONE_TIME_SNAPSHOT backfill (COMPLETE for `dashboard-tring`, 2026-07-20):** Snapshot request ID `f8470156-c123-49cf-860d-bed40475e688` retrieved via `GET /v1/apps/1350501409/analyticsReportRequests` (contrary to prior belief, this endpoint works and returns all existing requests). 6,188,432 rows loaded covering Nov 2024 - Jun 2026. Quota = 1 active snapshot per app at a time (HTTP 409 if old snapshot still active — not a per-month reset). Run `python -m tring_ingest --source app_store --snapshot` via Cloud Run Job (see runbook for full command). OOM fix: `load_tsv_stream_to_raw` streams TSV line-by-line in 10k-row batches — job needs 8Gi memory for snapshot run, revert to 4Gi after.
