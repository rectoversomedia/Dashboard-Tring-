...
1. Build the single `pipeline` container image and push it to Artifact Registry
2. Update all 5 existing Cloud Run Jobs (`extract-appsflyer`, `extract-moengage`, `extract-play-console`, `extract-app-store`, and `dbt-transform`) to use the new image
